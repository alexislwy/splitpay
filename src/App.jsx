cat > /home/claude/splitpay-firebase/src/App.jsx << 'ENDOFFILE'
...
const getMyId = () => { let id=localStorage.getItem(MY_ID_KEY); if(!id){id=uid();localStorage.setItem(MY_ID_KEY,id);} return id; };
...
function settleMixed(members, expenses, rates) {
  ...
  return {balances,hkdTxns,curTxns,allTxns:[...hkdTxns,...curTxns]};
}
...
function exportExcel(trip,members,expenses,rates){
  ...
  const expRows=(expenses||[]).map(e=>({
    ...
    "港幣換算":+((e.amount*(e.currency===BASE?1:(rates[e.currency]||1))).toFixed(2)),
    "付款人":mm[e.paidBy]?.name||"",
    "分攤成員":(e.splits||[]).map(s=>mm[s]?.name||"").join("、"),
    "人數":e.splits?.length||0,"結算幣值":e.settleMode==="original"?"原幣":"港幣",
  }));
  const balRows=members.map(m=>({成員:m.name,"餘額(HKD)":+fmt(balances[m.id]||0,2),狀態:(balances[m.id]||0)>0.01?"待收款":(balances[m.id]||0)<-0.01?"待付款":"已結清"}));
  const settleRows=allTxns.map(t=>({付款方:mm[t.from]?.name||"",收款方:mm[t.to]?.name||"",幣值:t.currency,金額:+t.amount.toFixed(2)}));
...
function exportPDF(trip,members,expenses,rates){
  ...
  <style>body{font-family:'Noto Sans TC',sans-serif;padding:32px;color:#1e293b;max-width:800px;margin:0 auto}h1{font-size:24px;color:#0ea5e9}h2{font-size:16px;color:#334155;border-bottom:2px solid #e2e8f0;padding-bottom:6px;margin-top:28px}table{width:100%;border-collapse:collapse;font-size:13px;margin-top:10px}th{background:#f1f5f9;padding:8px 10px;text-align:left;color:#475569;font-weight:600}td{padding:8px 10px;border-bottom:1px solid #f1f5f9}.green{color:#059669;font-weight:700}.red{color:#dc2626;font-weight:700}.tag{display:inline-block;background:#e0f2fe;color:#0369a1;border-radius:10px;padding:1px 8px;font-size:11px;margin:1px}@media print{body{padding:16px}}</style>
  </head><body>
  <h1>💸 ${trip.name}</h1><p style="color:#64748b;font-size:13px">生成：${new Date().toLocaleString("zh-TW")}</p>
  <h2>帳目（${(expenses||[]).length}筆）</h2>
  <table><thead><tr><th>日期</th><th>描述</th><th>金額</th><th>港幣</th><th>付款</th><th>分攤</th></tr></thead><tbody>
  ${(expenses||[]).map(e=>{const r=e.currency===BASE?1:(rates[e.currency]||1);return`<tr><td>${e.date||new Date(e.createdAt).toLocaleDateString("zh-TW")}</td><td>${e.desc}</td><td>${cur(e.currency)?.symbol}${e.amount.toLocaleString()}</td><td>HK$${fmt(e.amount*r)}</td><td><span class="tag">${mm[e.paidBy]?.name||""}</span></td><td>${(e.splits||[]).map(s=>`<span class="tag">${mm[s]?.name||""}</span>`).join("")}</td></tr>`;}).join("")}
  </tbody></table>
  <h2>個人餘額</h2>
  <table><thead><tr><th>成員</th><th>餘額(HKD)</th><th>狀態</th></tr></thead><tbody>
  ${members.map(m=>{const b=balances[m.id]||0;return`<tr><td>${m.name}</td><td class="${b>0.01?"green":b<-0.01?"red":""}">${b>0.01?"+":""}HK$${fmt(b)}</td><td>${b>0.01?"💰待收":b<-0.01?"💸待付":"✅已清"}</td></tr>`;}).join("")}
  </tbody></table>
  <h2>結算（${allTxns.length}筆）</h2>
  <table><thead><tr><th>付款方</th><th></th><th>收款方</th><th>金額</th></tr></thead><tbody>
  ${allTxns.map(t=>`<tr><td><b>${mm[t.from]?.name||""}</b></td><td style="color:#94a3b8">→</td><td><b>${mm[t.to]?.name||""}</b></td><td class="red">${cur(t.currency)?.symbol||""}${t.amount.toLocaleString()}</td></tr>`).join("")}
  ${allTxns.length===0?`<tr><td colspan="4" style="text-align:center;color:#059669;padding:16px">🎉已結清！</td></tr>`:""}
  </tbody></table></body></html>`;
  const w=window.open("","_blank");w.document.write(html);w.document.close();setTimeout(()=>w.print(),500);
}

// ── Edit Expense Modal ─────────────────────────────────────────────────────
function EditExpenseModal({exp,members,rates,onClose,onSave}){
  ...
  const [paidBy,setPaidBy]=useState(exp.paidBy);
  const [splits,setSplits]=useState(exp.splits||[]);
  const [date,setDate]=useState(exp.date||new Date(exp.createdAt).toISOString().split("T")[0]);
  ...
  const submit=()=>{
    ...
    onSave({...exp,desc:desc.trim(),amount:parseFloat(amount),currency,paidBy,splits,date});
  };
  return(
    <Modal title="修改帳目" onClose={onClose}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:currency!==BASE?4:14}}>
        <div>
          ...
          <input type="number" min="0" step="0.01" style={inp} value={amount} onChange={e=>setAmount(e.target.value)}/>
        </div>
        <div>
      ...
      <button onClick={submit} style={{...primaryBtn,width:"100%"}}>儲存修改</button>
    </Modal>
  );
}

// ── Add Expense Modal ──────────────────────────────────────────────────────
function AddExpenseModal({members,rates,myId,onClose,onAdd}){
  ...
  const submit=()=>{
    if(!desc.trim()||!amount||splits.length===0)return;
    onAdd({id:uid(),desc:desc.trim(),amount:parseFloat(amount),currency,paidBy,splits,date,settleMode:"hkd",createdAt:Date.now()});
  };
  return(
    <Modal title="新增帳目" onClose={onClose}>
      <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:10,marginBottom:14}}>
        <div>
          <div style={{fontSize:12,color:C.muted,marginBottom:6}}>描述</div>
          <input style={inp} placeholder="演唱會門票、晚餐、Uber…" value={desc} onChange={e=>setDesc(e.target.value)}/>
        </div>
        <div>
          <div style={{fontSize:12,color:C.muted,marginBottom:6}}>日期</div>
          <input type="date" value={date} onChange={e=>setDate(e.target.value)} style={{...inp,width:140}}/>
...
export default function App() {
  ...
  // Subscribe to current trip real-time
  useEffect(()=>{
    if(!tripId)return;
    setLoading(true);
    ...
    const unsub=onValue(r,snap=>{
      if(snap.exists()){
        ...
        setTrip({...data.meta,id:tripId,members,expenses,rates:data.rates||DEFAULT_RATES});
      } else setTrip(null);
      setLoading(false);
    });
    return()=>unsub();
  ...
  // FIX 1 & 3: use get() to directly fetch from Firebase by code
  const joinTrip=async(code,memberName)=>{
    try {
      const snap=await get(ref(db,`trips/${code}/meta`));
      if(!snap.exists()){alert("找不到此行程代碼，請確認代碼是否正確");return;}
      if(snap.val().archived){alert("此行程已封存");return;}
      // Check if already a member
      const memberSnap=await get(ref(db,`trips/${code}/members/${myId}`));
      const colorSnap=await get(ref(db,`trips/${code}/members`));
      const memberCount=colorSnap.exists()?Object.keys(colorSnap.val()).length:0;
      if(!memberSnap.exists()){
        const me={id:myId,name:memberName,color:COLORS[memberCount%COLORS.length]};
        await fbUpdate(`trips/${code}/members`,{[myId]:me});
      }
      addMyTrip(code); setTripId(code); setScreen("trip");
    } catch(e){alert("加入失敗，請稍後再試");}
  };
  ...
  // FIX 2: anyone can edit/delete any expense
    ...
    <div style={{minHeight:"100vh",background:C.bg,padding:24,fontFamily:"'Noto Sans TC',sans-serif"}}>
                  ...
                  <div key={id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:16,marginBottom:10}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                      <div>
                        ...
                        <div style={{fontSize:11,color:C.muted,marginTop:2,fontFamily:"monospace"}}>代碼：{id}</div>
                      </div>
      ...
      {showNewTrip&&(
        <Modal title="新增行程" onClose={()=>setShowNewTrip(false)}>
          ...
          <button onClick={async()=>{if(!newTripName.trim()||!myName.trim())return;await createTrip(newTripName.trim(),myName.trim());setShowNewTrip(false);setNewTripName("");setMyName("");}} style={{...primaryBtn,width:"100%"}}>建立行程 →</button>
        </Modal>
      )}
      ...
      {showJoin&&(
        <Modal title="加入行程" onClose={()=>setShowJoin(false)}>
          ...
          <div style={{marginBottom:8}}>
            ...
            <input style={inp} placeholder="朋友分享的代碼" value={joinCode} onChange={e=>setJoinCode(e.target.value.trim())}/>
          </div>
          <div style={{fontSize:11,color:C.muted,marginBottom:20}}>代碼可在行程頁「成員」分頁找到</div>
          <button onClick={async()=>{if(!myName.trim()||!joinCode.trim())return;await joinTrip(joinCode.trim(),myName.trim());setShowJoin(false);setJoinCode("");setMyName("");}} style={{...primaryBtn,width:"100%"}}>加入行程 →</button>
        </Modal>
      )}
    </div>
  );
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Noto Sans TC',sans-serif",paddingBottom:80}}>
          ...
          {[...expenses].sort((a,b)=>(b.date||"").localeCompare(a.date||"")).map(exp=>{
            ...
            return(
              <div key={exp.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"14px 16px",marginBottom:10}}>
                ...
                {/* FIX 2: All members can edit/delete any expense */}
                {!trip?.archived&&(
                  <div style={{display:"flex",gap:8,marginTop:4}}>
                    <button onClick={()=>setEditingExp(exp)} style={{...ghostBtn,padding:"5px 12px",fontSize:11,flex:1}}>✏️ 修改</button>
                    <button onClick={()=>{if(window.confirm("確定刪除此帳目？"))deleteExpense(exp.id);}} style={{...dangerBtn,padding:"5px 12px",fontSize:11,flex:1}}>🗑 刪除</button>
                  </div>
                )}
              </div>
            );
        ...
        {tab==="members"&&<>
          {!trip?.archived&&(
            <div style={{display:"flex",gap:8,marginBottom:16}}>
              <input style={{...inp,flex:1}} placeholder="新增成員名字" value={newMemberName} onChange={e=>setNewMemberName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&newMemberName.trim()&&addMember(newMemberName.trim()).then(()=>setNewMemberName(""))}/>
          ...
          {members.map(m=>(
            <div key={m.id} style={{display:"flex",alignItems:"center",gap:12,background:C.card,border:`1px solid ${m.id===myId?m.color+"44":C.border}`,borderRadius:12,padding:"12px 14px",marginBottom:8}}>
              <Avatar member={m}/>
              <div style={{flex:1}}><div style={{fontSize:14,color:C.text,fontWeight:600}}>{m.name}{m.id===myId&&" （我）"}</div></div>
            </div>
          ))}
          <div style={{marginTop:16,padding:14,background:"rgba(56,189,248,.07)",border:"1px solid rgba(56,189,248,.2)",borderRadius:12}}>
            ...
            <div style={{fontSize:12,color:C.muted,marginBottom:8}}>分享以下代碼，朋友選「加入行程」輸入即可即時同步。</div>
            <div style={{fontFamily:"monospace",fontSize:14,color:C.text,background:"rgba(0,0,0,.3)",padding:"10px 12px",borderRadius:8,wordBreak:"break-all",letterSpacing:.5}}>{tripId}</div>
        ...
        {tab==="export"&&<>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <button onClick={()=>exportExcel(trip,members,expenses,rates)} style={{...primaryBtn,display:"flex",alignItems:"center",justifyContent:"center",gap:10,fontSize:15,padding:16}}>
              <span style={{fontSize:24}}>📊</span><div style={{textAlign:"left"}}><div>匯出 Excel</div><div style={{fontSize:11,opacity:.7,fontWeight:400}}>3個工作表</div></div>
            </button>
            <button onClick={()=>exportPDF(trip,members,expenses,rates)} style={{...ghostBtn,display:"flex",alignItems:"center",justifyContent:"center",gap:10,fontSize:15,padding:16,borderRadius:12}}>
              <span style={{fontSize:24}}>📄</span><div style={{textAlign:"left"}}><div>匯出 / 列印 PDF</div><div style={{fontSize:11,opacity:.6,fontWeight:400}}>開啟瀏覽器列印</div></div>
            </button>
          </div>
      ...
      {showAddExp&&<AddExpenseModal members={members} rates={rates} myId={myId} onClose={()=>setShowAddExp(false)} onAdd={async exp=>{await addExpense(exp);setShowAddExp(false);}}/>}
      {editingExp&&<EditExpenseModal exp={editingExp} members={members} rates={rates} onClose={()=>setEditingExp(null)} onSave={async exp=>{await updateExpense(exp);setEditingExp(null);}}/>}

      {showRates&&(
        <Modal title="匯率（1外幣 = ? HKD）" onClose={()=>setShowRates(false)} wide>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <div style={{fontSize:12,color:C.muted}}>可手動修改（信用卡匯率）</div>
...
ENDOFFILE
echo "Done: $(wc -l < /home/claude/splitpay-firebase/src/App.jsx) lines"
